...
**QL Kernel names the whole canonical QL object, not merely the Rust `ql-core` crate.** Questioning, archetypal numericality, Logos, Vāk, harmonic and geometric determination, Meta Epistemic Framework refraction, Context-Frame composition and generative use disclose one field. This document reconciles their current representations; it does not add a super-kernel or another shape ontology.

The articulation square is:

```text
L0   Quaternal                 L0′  Archetypal-Numerical
L5′  Divine Logos             L5   Para Vāk
```

**L5 is the concrescence of L0/L0′/L5′/L5**, not a specialised speech subsystem surrounding an independent kernel. Concrescence here means that questioning, numerical determination and Logos become articulated together, not that a fourth software service aggregates three other services. The four vertices already exist in `LensId` and `MefSquare::Articulation`.

The faithful L5 projection is:

| Existing local address | Office | What it determines |
|---|---|---|
| `mef:sublens:L5.0@1` | **Syntax** | Full Anuttara/Vāk language and its source-grounded operative articulation |
| `mef:sublens:L5.1@1` | **Root** | **QL seed/core = psychoids + coordinate-family heads** |
| `mef:sublens:L5.2@1` | **Harmonics** | Efficient/moving, ratio, resonance and musical determination; Context Frames already active |
| `mef:sublens:L5.3@1` | **Geometry** | Formal/topological disclosure of the same relations |
| `mef:sublens:L5.4@1` | **Meta Epistemic Framework** | Refractive/contextual reading of the same QL Kernel |
| `mef:sublens:L5.5@1` | **Techne** | Generative use of the whole, producing determinations capable of Return as new wholes/grounds |

These office names do **not** rename source Anuttara, Para Vāk or the canonical Q2 sublens labels `Anuttara/Asambhava`, `Para Vāk`, `Paśyantī`, `Madhyamā`, `Vaikharī`, `Mātṛkā`. `l5_projection::L5Office` supplies the office name, the existing `SublensRef`, and the separate source label. There are still twelve lenses and seventy-two sublenses, not six new packages or extra semantic nodes.

`L5-2` means local position 2 in this projection. The existing rotation is `absolute = (lens.index + local) mod 6`; thus L5 local 2 rotates to absolute 1. The office reading is not a reason to change rotation, and source parallels at local position 2 are not assertions that all those occurrences share an absolute address.

## 2. Actual inspected basis and authority

The following are inspected commit bases, not floating runtime-selection pins:

| Repository | Inspected main / source revision | Material inspected |
|---|---|---|
| QL-MEF | `44ed3cd0e7a8bc25508a4e18ad3bb4c730013913` | QL addresses/relations; MEF registry/rotation; Context Frames and external readings; harmonic cuts; Vāk source, native evidence, Wiki contracts and acceptance history |
| AIKit | `59dae3f62fdf9d1a62638e2f7b6857d46692fd6c` | `crates/aikit-core/src/resource/operative.rs`; accepted #142 Search/Resolve programme |
| Software Factory | `885f3c29726dc6cb9397eca246abdfb1c65eff86` | `factory/src/orchestration.rs` and `factory/src/journey_praxis.rs`; existing native developmental/execution correlations |
| Epi-Logos-C-Experiments | `daa660cbc1b8c5da83828698665a753852cb0287` | Anuttara source lock; C reference bodies; Ta-Onta address, Anima contract and spine code; retained source is read-only in this track |

QL source authorities are exact refs and laws, not whatever implementation is easiest to inspect. The Anuttara snapshot is `data/epi-bimba-map/anuttara-language-map.md`, originating at `Idea/Bimba/Map/datasets/anuttara-deep/anuttara-language-map.md`, source blob `22835042d4d2c4ba821c252bd4fbfe52f39712ef`. `vak.rs` retains its provenance and all 109 entries. Source absences and provisional statements remain absences and provisional statements; preserved metaphysical formulation is not empirical verification.
...
[#83](https://github.com/EpiLogos/QL-MEF/issues/83) / PR #84 is accepted, with merge `230ea85d732fd8d6f434767a495e5bdb02c36cc4` and receipt [EPI-VAK-ACCEPTANCE-RECEIPT.md](integrations/epi-logos/EPI-VAK-ACCEPTANCE-RECEIPT.md). Its four-owner verification includes source parity, actual AIKit operative paths, native Factory/Central Actions, observed Activity/Return and derived Recognition. Recorded run `33663063880` is prior evidence, **not** this reconciliation's verification run.

The receipt's AIKit `4e35f499c50b987551ab124b4432757973e823ae`, Factory `71287c179e7686cb37cb25267958f0c390a1ecb7` and Central `77625f40081a3e38503d732f44c607a94888d9d6` are immutable proving revisions, not declarations of current main. Accepted AIKit [#142](https://github.com/EpiLogos/ai-kit/issues/142) is likewise inherited. Full-scale recursive C′ orchestration is a further frontier, not grounds to demote this returned implementation to a symbol registry.

## 3. Syntax: full Vāk and one general operative grammar

The source field and the general expression surface have different responsibilities without becoming separate competing languages.

| Surface | Current identity / purpose | Boundary |
|---|---|---|
| Anuttara M0, all 109 nodes | Source-grounded language, formulations, hierarchy and explicit relations | Never reduce the whole to the most convenient six glyphs |
| M0-3 | Mirror/Frame/Operator, archetypal numbers, twelve self↔other forms, Mono/Poly and R-action articulation | Semantic reading/generation is not a regex truth judgement |
| M0-4 | Holographic contextual/relation field | A field correspondence is not native ownership or Action permission |
| M0-5 | Śiva relation operations and Śakti addressing horizons | Shared numeric order is not identity with other sixfolds |
| `VakRef` / `VakRegistry` | Address and validate source entries; retain source revision and field provenance | Syntactically shaped address is not proof that a source node exists |
| AIKit `ResolveExpression` | Provider-neutral Subject, Address, Unary, Binary and nested Frame; parse/render/resolve | One generic AST; QL does not copy its parser |
| QL `VakExpressionReadingV1` / `VakPathV1` | Source-grounded interpretation and attributable operative/execution evidence | A semantic reading is not an execution receipt; an observed path needs actual evidence |
| Native Action → Invocation → Activity → Return | Consequential operation under its native owner | Addressability, availability, authority and actual execution remain distinct |

The accepted minimal projection remains:

```text
Śiva / RelationOp       @#  -  +  x  /  =
Śakti / AddressHorizon  @0 @1 @2 @3 @4 @5
```

AIKit owns `aikit.operative-resolve/v1`. Ordinary text search, situated resolution, ranking/familiarity, KnowledgeRoute and Method-classified Skill use must continue without requiring a QL provider. A full QL profile enriches the **same** expression with source-qualified readings, scopes and evidence; it does not introduce another Factory/Wiki parser.
...
```text
# / 0/1 ↔ 1/0
    raw psychoid / positioned possibility #0 … #5
    coordinate-family heads C / P / L / S / T / M
    direct / conjugate capacity
```

`QlFamily::{C,P,L,S,T,M}` are the existing six family heads. `None` is the pre-categorical/no-family case, not a seventh peer family. The historical `psychoid_numbers.h` distinguishes the psychoid bedrock from family manifestations. Root is this seed/core, not the whole kernel package.
...
| Sixfold | What the indices belong to |
|---|---|
| C/P/L/S/T/M | Coordinate-family heads |
| CPF/CT/CP/CF/CFP/CS | Reflective Vāk/contextual families |
| `@# - + x / =` | Relation operations |
| `@0 … @5` | Addressing horizons |
| Syntax … Techne | Local L5 offices |
| Khora … Aletheia | S′ Agent-world composition offices |

Their relations may be derived or source-bound. Their shared cardinality does not license casting one enum into another. In particular reflective `VakFamily::Cf` is slot 3 while **Context Frames are already active at L5 office 2**. Neither index changes the other's identity.

## 5. Harmonics and active Context Frames

The moving office is not a synonym for a rendered shape. At local position 2, the existing MEF registry preserves **Efficient cause** (L1), **Feeling** (L1′), **Water** (L2′) and **Dynamis** (L5′). These are source/architectural corroborations for movement, ratio and resonance, not a numerical proof that every interpretation of 2 is interchangeable.

There is executable evidence beyond labels. `music::cf_diatonic_cut` consumes `ContextFrameCut::canonical`, retaining its frame identities and Name/Power form while deriving harmonic pitches. `mode_tonic_instance` and `mode_tonic_landscape` develop that same cut. Twelve lenses × seven modes gives eighty-four mode/tonic instances per basis; that is not a replacement for the twelve × six = seventy-two MEF addresses. Chromatic/fifths bases and pitch realisations are determinations over the canonical relations, not new Context Frames.

The seven canonical expressions, in current CF1–CF7 order, are:
...
`VakFamily::Cf.relation_id()` is the existing `KernelRelationId::ContextFrame`, identified by `ql.kernel.context-frame/v1`. Preserve the historical C zero-based table/current CF1–CF7 representation distinction when proving parity.

`ExternalSixfoldMapping` already carries target/source identity, exact external position refs and a mapping digest. `ExternalContextFrameReading` preserves provider, operator version, origin, probe, evidence and **Exact / Partial / Ambiguous / NoReading**. It explicitly cannot confer runtime authority. These are existing building blocks, not grounds for another untyped CF metadata bag.

The architectural propagation law is:

```text
harmonic/contextual determination: active CF + source/binding provenance
    → geometric disclosure of the same framed relation
    → Meta Epistemic Framework reading of that same determination
    → Techne act under the same explicit frame/scope
...
Geometry is formal/topological determination: partial constellations, cardinality, D-developed fields, quaternity, circles, torus/Klein/Möbius/fold/wave forms and renderable topology. It is not a second ontology and is not interchangeable with the ratio/resonance/movement determined at Harmonics. Existing formal relation IDs must survive into any geometry or C++ embodiment; interpolated/rendered state cannot silently become exact symbolic canon. Deep embodiment belongs to #129–#133, not this projection module.

[The Wiki structural v2 contract](wiki-structural-contract-v2.md) fixes the within-face pairs:
