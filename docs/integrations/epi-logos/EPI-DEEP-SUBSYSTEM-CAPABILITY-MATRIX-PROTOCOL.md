...
The unit is a **real Epi capability**, not a technology or repository.

```text
FOR WHAT?
→ M meaning / M′ lived instrument
→ Agent-world requirement
→ S′ / Ta-Onta composition
→ current S / O:I native owners
...
M1 Paramaśiva   S1 Actuation           S1′ Hen
M2 Paraśakti    S2 AIKit               S2′ Pleroma
M3 Mahāmāyā     S3 Software Factory    S3′ Chronos
M4 Nara         S4 Workcell            S4′ Anima
...
Every capability row should answer:

```text
capability_ref
for_what
M determination + source
M′ lived/deep operation
S′ coordinates participating
current O:I native owners
providers / versions
implementation standing
observed evidence
source / observation / inference / proposal distinction
...
Before adding Epi machinery ask:

1. Does the generic capability remain complete when Epi meaning is removed? If yes, its mechanics belong to the native O:I owner.
2. What extra relation is required for that capability to participate in Bimba/M/M′, Vāk/CF, Epi provenance, lived context and Return? That is the S′/Epi composition.

Current generic centres:

- Central — durable human/Project source ground.
- Actuation — Agent, Agency, determination, authority and Return.
- AIKit — Context, Knowledge, SemanticWiki, Capability, Skill/Method, model/harness, SessionSpace and Surfaces.
- Factory — Project, Commission/Journey/Run, Artifact/Evidence/Candidate/Recognition.
- Workcell — material provider/body/process/service/store.
- QL-MEF — QL relations, MEF/refraction, Vāk and Context-Frame form.
- O:I parent — Projection, Participant, SharedField, Encounter and transport.

Ta-Onta composes these; it does not duplicate their registries.

## 4. Six S′ questions

```text
S0′ Khora     what ground, entry and continuity does the Agent need?
S1′ Hen       what Epi semantic/artifact form must it inhabit?
S2′ Pleroma   what powers, models and instruments constitute the operative affordance field?
S3′ Chronos   what becoming, clock, Kerykeion/Kairos and collective/personal NOW qualify it?
S4′ Anima     how does it become situated Agency under Vāk/CF/authority/material conditions?
S5′ Aletheia  what difference is disclosed and how can it become Knowledge/Evidence/Return?
```

Run all six questions against all six M domains. Empty cells require an explicit reason, not omission by habit.
...
A model/harness/adapter is a generic operative-body capability. AIKit/Workcell expose it; Pleroma constitutes available power; Anima situates it as Agency; Factory can return Run/Evidence.

Subsystem-specific development resolves through the already-authored **M5′ Epii-on-X operational-capacity family**:

```text
M0 Epii-on-Anuttara    language development — construction, not training
M1 Epii-on-Paramaśiva  foundational-derivational CPT + RAG
M2 Epii-on-Paraśakti   graph-relational ML over derived geometry
M3 Epii-on-Mahāmāyā    process-reward RL + federated + symbolic/genetic pathway synthesis
M4 Epii-on-Nara        Anima-led narrow QLoRA voice + optional DPO
...
```text
actual activity
→ observation / evidence / WikiReading
→ Aletheia/Epii interpretation
→ candidate/proposal
→ Recognition by the authority owning target ground
→ accepted revision or explicit refusal
→ renewed ground
```
