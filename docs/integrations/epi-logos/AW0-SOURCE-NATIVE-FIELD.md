...
149 deeper matrix capabilities and their 33 unresolved/open/drift conditions;
all 109 Vāk source entries and the distinct 36 operation/horizon profiles.
It also enumerates the six C′ offices, seven settled CFs, six CFP forms plus Z,
six paired CS walks, two participation regimes, seven content forms, six context
positions, all twelve T/T′ meanings, the six property offices and six Epii-on-X
laws. Equal cardinalities do not collapse these inventories into one another.

Original participation retains 15 agent-definition files, eight specialist/mode
readings (Moirai is one actor with three modes), all 65 Skill files including
staged copies, six agent-definition offices, 82 source manifest members including
full Skill profiles and authority restrictions, and all ten original operational
jobs. Their historical source is not an instruction to recreate obsolete native
products, restore incorrect CT/CS meanings, or grant authority.

The property census executes the original graph-schema registry: **194 definitions,
164 node and 30 relationship**. It also covers **5,686 property-key paths in 85,474
occurrences** in the retained M source records. Each occurrence resolves back to
its original file, record index and payload digest through the existing manifest.
A payload key is not an implicitly canonical property type. The six recovered
producer families retain QV read/write, knowing, bake, relationship writes and
GDS projection separately. Prefixes such as `q_4_` neither select a local office
nor prove that a synthesis algorithm exists.

## Provenance and retrieval

`aw0-source-basis.json` pins the inspected native revisions and 206 individual
source blobs. Its four readable members partition native APIs, original laws,
Skills and agent definitions. Paths are retained exactly; the historical source
root remains C-Experiments `daa660cbc1b8c5da83828698665a753852cb0287`.

`aw-property-source.json.gz` is a deterministic gzip of ordinary JSON exported by
executing the original Rust `NODE_PROPERTY_SPECS` and
`RELATIONSHIP_PROPERTY_SPECS` arrays. It includes their semantic registry and
source identity. `aw-original-participation.json.gz` is a deterministic captured
JSON projection of the pinned Pleroma source manifest, retaining original fields
and current correction standing. Both are readable with `gzip -dc`; compression
is packaging, not an additional semantic wire format. Nothing loads these source
captures as a new runtime registry or a permission policy.

The source check is `python3 scripts/aw0.py --verify-sources <parent>` where the
parent contains the exact owner checkouts named by the source-basis file.
