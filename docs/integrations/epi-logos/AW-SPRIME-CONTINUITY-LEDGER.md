...
- **Dated note, 2026-09-24 — G2 addressed at its native owners.** On the owner's instruction the six specialists, with Aletheia as their lead, are defined as the Central agent set `aletheia` (and the constitutional field as the agent set `anima`), one Central AgentProfile per member, member operating instructions under `Control/agents/expressions/{anima,aletheia}/members/`, and the domain skills they use ported into `skills/` (`skill/ql/anansi`, `skill/ql/gnosis-retrieve`, `skill/ql/thought-distil`, the six `aletheia-*-gate` skills and the rest). The census rows above are unchanged; they are still source coverage, not runtime acceptance. No QL ontology change. See [`ANIMA-ALETHEIA-TEAMS.md`](ANIMA-ALETHEIA-TEAMS.md).
...
|---|---|---|---|---|
| G1 | S4′ CFP-form / Z / CS-walk surface | **CLOSED 2026-09-17** (commit `3172049`): `cfp-form` / `z-begin` / `z-advance` / `z-reopen` / `cs-select` / `cs-hop` / `inspect-oikonomia` / `operative-return` CLI ops, fixture `fixtures/kernel/vak-cprime-oikonomia-v1.json` performing the full CS0 forward traverse with the form and passage markers proven to survive into the operative Return; tests pin pair-direction rejection and Z stage ordering. Native conduct through real Units/Attempts remains G4 (Factory #217) | ~~QL-MEF (CLI/facet surface), with Factory #217 for native conduct~~ | ~~Add `cfp-form` / `z-begin` / `z-advance` / `cs-select` / `cs-hop` ops to `ql-cli/src/vak_composition.rs` dispatch and one fixture composition that walks a full CS0 profile through positioned Returns~~ |
...
**G2 update, 2026-09-24 (added; the row above is kept as recorded on 2026-09-17).** The owner instructed that Anima and Aletheia be built as native agent sets now, relative to Factory workflows and without waiting for the QL prime agent loop. Done at the native owners: Central agent sets `anima` and `aletheia` and fifteen AgentProfiles (proposal inputs; recognition is the owner's act), member expressions in `Control/agents/expressions/`, and 26 team skills in `skills/`. Remaining: the proposals must be proposed and recognised in Central, AIKit must sync its `ql` source to a revision containing the skills, and AIKit's projection of member files into `.claude/agents/` is a separate AIKit change. Unbuilt tool replacements (arena routing, personal Kairos, oracle-spread aliveness, Klein weighting, the symbolic-protein reader) are specified in `ANIMA-ALETHEIA-TEAMS.md` §4.
