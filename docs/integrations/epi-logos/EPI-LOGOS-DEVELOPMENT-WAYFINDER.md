...
O:I desktop / agent-IDE host
        ↓
minimum existing Epi kernel + primitive bridge
        ↓
M4′ Nara first lived Pratibimba vertical
        ↓
Personal 4/5/0: Nara + Epii + Anuttara/Bimba
        ↓
Cosmic M1′ + M2′ + M3′ as one integrated instrument
        ↓
full M0′–M5′ deep workspaces
        ↓
...
PRATIBIMBA / EPI MODE
│
├─ Personal — primary daily lived face
...
M4′ Nara
  lived condition / episode / question
        ↓
M5′ Epii
  pedagogy / explanation / review / proposal / return
        ↓
M0′ Anuttara / Bimba
...
Epi `Body/S/S0/portal-core`   Rust domain/projection implementation
        ↓
...
O:I desktop starts
  ↓
Epi / Pratibimba is hosted as a native mode/contribution
  ↓
real Epi kernel/library is reachable through a typed bridge
  ↓
M4′ Nara daily surface opens
  ↓
user writes a protected journal/day episode
  ↓
DAY/NOW + harmonic/VAK/coordinate context comes from real shared state
  ↓
O:I situated agent and canvas co-refer to the same Nara object/context
  ↓
highlight/sendoff dispatches a governed Action
  ↓
Epii/Anuttara explanation/review can be summoned
  ↓
