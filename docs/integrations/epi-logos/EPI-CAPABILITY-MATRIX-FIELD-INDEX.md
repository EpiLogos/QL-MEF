...
1. `EPI-DEEP-SUBSYSTEM-CAPABILITY-MATRIX-PROTOCOL.md` — method, provenance and standing discipline.
2. `EPI-TA-ONTA-AGENT-WORLD-CAPABILITY-MATRIX.md` — the 6 × 6 M × S′ inhabitation field.
3. `EPI-M-CAPABILITY-FIELD.md` — the 36 recovered real capabilities, now read through M′ operation, S′ composition, native O:I embodiment, Epii-on-X development and Return.
4. `epi-m-capability-field.json` + `epi-m-capability-field-m0.json` … `epi-m-capability-field-m5.json` — normalized machine carrier/evidence registry.
5. `epi-epii-operational-capacities.json` — exact six source-backed M5′ Epii-on-X developmental relations.
6. `EPI-CAPABILITY-MATRIX-SOURCE-TRACE.md` — current locks, exact source blobs, live implementation deltas and historical lineage.
