...
impl FromStr for QlAddress {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        let frame = parts.next().ok_or_else(|| QlError::InvalidAddress(value.to_owned()))?;
        let face = parts.next().ok_or_else(|| QlError::InvalidAddress(value.to_owned()))?;
        let position = parts.next().ok_or_else(|| QlError::InvalidAddress(value.to_owned()))?;
        let depth = parts.next().ok_or_else(|| QlError::InvalidAddress(value.to_owned()))?;
