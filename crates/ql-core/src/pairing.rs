...
impl CanonicalCrossPass {
    /// Canonical operator identity shared by C and Rust.
    ///
    /// D1/D2/D3 remain the derivational coordinates of this formal construction;
    /// callers identify the operation itself by the semantic kernel relation.
    pub fn operator_ref(&self) -> String {
        self.kernel_relation_id().as_str().to_owned()
    }

    /// Provenance-bearing derivation reference, retaining the exact Q6 D-coordinate
    /// by which the canonical relation was constructed.
...
/// Build one selected square in the canonical A/B/C x three-pair apparatus.
///
/// D1/D2/D3 are the degree of square completion:
...
/// Canonical cross-position family.
///
/// `transform`: `(n, (n+1)')`
/// `require`: `(n, (n-1)')`
/// `complete`: `(n, (5-n)')`
pub fn canonical_cross_pass_d2(kind: D2CrossPassKind, position: QlPosition) -> CanonicalCrossPass {
    ...
    let conjugate_position =
        QlPosition::new(conjugate_position).expect("canonical cross relation remains inside 0..5");
...
/// Canonical A/B/C invariance on the conjugate face, preserving family and pair provenance.
