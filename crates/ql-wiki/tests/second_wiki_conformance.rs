...
const RESEARCH_LOCAL: &str = include_str!("../../../fixtures/qw4/research-wiki/question-local-space.md");
const RESEARCH_QUESTION: &str = include_str!("../../../fixtures/qw4/research-wiki/question-node.md");
const RESEARCH_EVIDENCE: &str = include_str!("../../../fixtures/qw4/research-wiki/evidence-node.md");
...
fn research_canvas_is_a_distinct_open_wiki_profile_with_recursive_node_as_whole() {
    ...
    assert_eq!(question.wiki.string("local_space_ref"), Some("research:space:question-local"));
    assert_eq!(local.wiki.string("anchor_ref"), Some("research:node:question"));
    assert_eq!(space.wiki.refs("child_space_refs"), vec!["research:space:question-local"]);
    assert_eq!(frame.wiki.canonical_ref, "research:frame:transition-anomaly");
    assert_eq!(frame.wiki.revision, 5);
    assert_eq!(evidence.wiki.raw["research_extension"]["observation_count"], 5);
}

#[test]
fn research_frame_consumes_exact_qw0_d3_and_return_contract_without_translation() {
    let frame = parse_okf_wiki(RESEARCH_FRAME).unwrap();
    let structural: WikiStructuralField = serde_json::from_value(
        frame.wiki.raw["research_extension"]["structural_field"].clone(),
    )
    .unwrap();
    ...
    assert_eq!(constellations[0]["anchor_ref"], "research:anchor:transition-whole");
    let return_path = &frame.wiki.raw["research_extension"]["return"];
    assert_eq!(return_path["through_anchor_ref"], "research:anchor:transition-whole");
...
fn real_registry_provider_refracts_second_wiki_whole_field_through_multiple_lenses() {
    ...
    assert_eq!(response.target_snapshot_hash, "sha256:research-transition-anomaly-v5");
    assert_eq!(response.readings.len(), 3);
    assert!(response.readings.iter().all(|reading| reading.target_revision.as_ref().unwrap().to_string() == "5"));
    ...
    assert!(disclosures.iter().any(|value| value.contains("Archetypal-Numerical")));
    assert!(disclosures.iter().any(|value| value.contains("Divine Logos")));
    assert!(response.readings.iter().all(|reading| {
...
fn invalid_second_wiki_operator_is_validation_error_before_provider_state() {
    ...
    request.target.structural_field.as_mut().unwrap().operator_ref =
        "ql:structural:2.0.0:field:B:0:D3".into();
...
fn proposed_external_feedback_does_not_mutate_projection_then_recognition_opens_new_route() {
    let mut projection = MetaKnowledgeProjection::rebuild(&meta_documents(), &bindings(), 1).unwrap();
    ...
    let candidate = ledger
        .propose(ExternalObservation {
            ...
            rationale: "five repeated process readings need explicit structural-field context".into(),
            occurrences: 5,
    ...
    assert!(!projection.relations.iter().any(|relation| {
        relation.relation == "contextualised-by-structural-field"
    }));
    ...
    assert!(route.steps.iter().any(|step| {
        step.relation == "contextualised-by-structural-field"
            && step.origin == "recognised"
    }));
    assert!(route
        .steps
        .iter()
...
fn recognised_amendment_can_be_reapplied_after_projection_rebuild_and_reverse_traversal_works() {
    ...
    assert!(reverse.routes.iter().flat_map(|route| route.steps.iter()).any(|step| {
        step.relation == "inverse:contextualised-by-structural-field"
    }));
}
