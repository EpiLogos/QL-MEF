...
impl World {
    fn new() -> Self {
        let root = std::env::temp_dir().join(format!("ql-nara-reuse-{}-{}-{}", std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos(), ID.fetch_add(1, Ordering::SeqCst)));
        Self(root)
    }
    fn call(&self, request: Value) -> Value {
        let mut child = Command::new(env!("CARGO_BIN_EXE_ql"))
            ...
            .spawn().expect("start selected native ql");
        child.stdin.take().unwrap().write_all(serde_json::to_string(&request).unwrap().as_bytes()).unwrap();
        let output = child.wait_with_output().unwrap();
        assert!(output.status.success(), "native CLI failure: {}", String::from_utf8_lossy(&output.stderr));
        serde_json::from_slice(&output.stdout).expect("native JSON reply")
    }
}
impl Drop for World { fn drop(&mut self) { let _ = std::fs::remove_dir_all(&self.0); } }
fn source(name: &str) -> Value { json!({"source_ref":format!("source:{name}"),"revision":"r1","standing_ref":"controlled-test"}) }
fn consent() -> Value { json!({"actor_ref":"person:controlled","actor_kind":"human","personal_data":true}) }
fn seed() -> Value {
...
fn applied(world: &World, record: &Value, id: &str, mutation: Value) -> Value {
    ...
    assert_eq!(reply["record"]["revision"].as_u64(), record["revision"].as_u64().map(|v|v+1));
    reply["record"].clone()
}
fn open(world: &World) -> Value {
    let reply = world.call(json!({"operation":"create","request_id":"create","consent":consent(),"seed":seed()}));
    assert_eq!(reply["ok"], true, "native create: {reply}");
    applied(world, &reply["record"], "start", json!({"kind":"practice_start","phase":{
...
fn phase(record: &Value) -> &Value { &record["domain"]["transformation"]["phase_history"]["phases"][0] }
#[test]
fn one_source_can_support_distinct_hold_resume_close_acts_without_duplicate_links() {
    let world = World::new(); let mut record = open(&world);
    for n in 0..3 {
        record = applied(&world, &record, &format!("hold:{n}"), json!({"kind":"practice_hold","phase_ref":"practice:one","feedback_ref":"central:source:feedback"}));
        assert_eq!(phase(&record)["safety"], "consent-required");
        record = applied(&world, &record, &format!("resume:{n}"), json!({"kind":"practice_resume","phase_ref":"practice:one","safety_review_ref":"central:source:feedback"}));
        assert_eq!(phase(&record)["safety"], "clear");
    }
    let request = apply_request(&record,"close",json!({"kind":"practice_close","phase_ref":"practice:one","feedback_refs":["central:source:feedback","central:source:new","central:source:new"]}));
    let reply = world.call(request.clone());
    assert_eq!(reply["ok"],true,"native close: {reply}"); record = reply["record"].clone();
    assert_eq!(phase(&record)["feedback_refs"],json!(["central:source:feedback","central:source:new"]));
    assert!(phase(&record)["closed_at_unix_ms"].is_u64());
    assert_eq!(record["receipts"].as_array().unwrap().len(),9,"one receipt for each real transition, not one per source");
    let replay = world.call(request);
    assert_eq!(replay["duplicate"],true); assert_eq!(replay["record"],record,"replay cannot create another occurrence");
    let restored = world.call(json!({"operation":"read","target":record["target"],"consent":consent()}));
    assert_eq!(restored["record"],record,"independent native process reopens exact private history");
    ...
    assert_eq!(world.call(json!({"operation":"read","target":record["target"],"consent":consent()}))["record"],record);
}
#[test]
fn reference_reuse_does_not_relax_resume_safety_or_input_validation() {
    let world = World::new(); let record = open(&world);
    ...
    assert_eq!(invalid["ok"],false,"a clear phase cannot fabricate a resumed hold");
    let invalid = world.call(apply_request(&record,"blank",json!({"kind":"practice_hold","phase_ref":"practice:one","feedback_ref":""})));
    assert_eq!(invalid["ok"],false);
    assert_eq!(world.call(json!({"operation":"read","target":record["target"],"consent":consent()}))["record"],record,"rejected actions do not mutate native state");
}
