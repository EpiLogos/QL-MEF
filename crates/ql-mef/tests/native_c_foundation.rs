...
use ql_core::{
    ...
    KernelRelationId, QlCoordinate, QlFace, QlFamily, QlPosition, RelationFamily,
    VakFamily, VakInstruction, canonical_cross_pass_d2, canonical_cross_pass_d3,
};
use ql_mef::{
    ContextFrameId, LensId, MefGrain, MefRotation, MefUnitFace, MusicalBasis,
    SublensRef, pitch_at_lens,
};

const COUNTS: &str = include_str!("../../../fixtures/kernel/native-c-foundation-counts-v1.tsv");

fn family(code: &str) -> QlFamily {
    QlFamily::ALL.into_iter().find(|f| f.code() == code).unwrap()
}
...
fn relation(code: &str) -> KernelRelationId {
    KernelRelationId::ALL.into_iter().find(|r| r.as_str() == code).unwrap()
}
...
fn target(r: KernelRelationId, f: QlFamily, c: QlCoordinate, other: QlFamily) -> String {
    ...
    match r {
        ...
        | ConjugateInvarianceC => {
            ...
            let invariant = matches!(r, ConjugateInvarianceA | ConjugateInvarianceB | ConjugateInvarianceC);
            ...
            to.position = pairs.into_iter().find_map(|[a, b]| {
                if a.position == c.position { Some(b.position) }
                else if b.position == c.position { Some(a.position) }
                else { None }
            }).unwrap();
        }
        CrossTransform | CrossRequire | CrossComplete => {
            ...
            let CanonicalCrossPass::D2 { coordinates, .. } = canonical_cross_pass_d2(kind, c.position) else {
                unreachable!()
            };
        ...
        PositionSuccessor => {
            if c.position.value() == 5 { return "-".into(); }
...
fn check_row(c: &[&str]) {
    match c[0] {
        ...
        "hash-edge" => {
            let expected = if matches!(relation(c[1]), KernelRelationId::PositionIdentity | KernelRelationId::SourceProvenance) { "#" } else { "-" };
            assert_eq!(c[2], expected);
        }
        "address" => {
            let coordinate = QlCoordinate::new(position(c[2]), face(c[3]));
            assert_eq!(c[4], label(family(c[1]), coordinate));
            assert_eq!(c[5], MusicalBasis::Chromatic.pitch_at(coordinate).to_string());
        }
        "edge" => {
            let r = relation(c[1]);
            if r != KernelRelationId::FamilySamePosition { assert_eq!(c[5], "NONE"); }
        ...
        "mef" => {
            ...
            assert_eq!(c[3], MefRotation::new(lens, local).absolute_position().value().to_string());
            assert_eq!(c[4], SublensRef::canonical(lens, local.value()).unwrap().to_string());
            let pitch = pitch_at_lens(MusicalBasis::Chromatic, lens, QlCoordinate::new(local, QlFace::Direct));
            assert_eq!(c[5], pitch.to_string());
        }
        "cf" => {
            let frame = ContextFrameId::ALL.into_iter().find(|f| f.code() == c[1]).unwrap();
            let coordinate = frame.canonical_selection().at_lens(c[2].parse().unwrap()).coordinate();
            assert_eq!(c[3], coordinate.local_position().value().to_string());
            assert_eq!(c[4], coordinate.absolute_position().value().to_string());
            assert_eq!(c[5], match coordinate.unit_face() { MefUnitFace::Name => "name", MefUnitFace::Power => "power" });
            assert_eq!(c[6], match coordinate.grain() { MefGrain::InnerFour => "inner-four", MefGrain::OuterTwo => "outer-two" });
            assert_eq!(c[7], frame.expression());
        }
        "vak" => {
            let vak = VakFamily::ALL.into_iter().find(|v| v.code() == c[1]).unwrap();
        ...
        "instruction" => {
            let vak = VakFamily::ALL.into_iter().find(|v| v.value().to_string() == c[1]).unwrap();
            assert_eq!(c[2], "255");
            let instruction = VakInstruction::new(vak, 255, c[3].parse().unwrap(), c[4].parse().unwrap(), face(c[5]) == QlFace::Conjugate).unwrap();
...
fn native_c_matches_current_rust_foundation() {
    ...
    let compile = Command::new(cc).current_dir(&root)
        .args(["-std=c11", "-Wall", "-Wextra", "-Werror", "-pedantic", "-Ic/include",
               "migration/epi-kernel/k1-foundation-probe.c", "c/src/primitive.c",
               "c/src/holographic.c", "c/src/kernel.c", "-lm", "-o"])
        .arg(&binary).output().expect("K1 parity requires a working C11 compiler (CC)");
    ...
    let expected: BTreeMap<_, _> = COUNTS.lines().filter(|l| !l.starts_with('#'))
        .map(|l| { let (kind, count) = l.split_once('\t').unwrap(); (kind, count.parse::<usize>().unwrap()) })
        .collect();
    assert_eq!(counts, expected, "native parity stream must be exhaustive, not a skipped subset");
