...
use ql_mef::m2_condition::{
    CorrespondenceRole, TuningPolicy, condition_pitches, correspondence_field,
};
...
fn musical_input() -> CoupledInput {
    ...
    request.condition_frequency_bindings = (0..8)
        .map(|i| ConditionFrequencyBinding {
            mode_ref: format!("controlled:musical-mode/{}", i + 1),
            pitch_index: i,
        })
        .collect();
    request
...
fn v2_binds_both_tunings_over_every_present_source_path_without_replacing_vimarsha() {
    ...
    assert_eq!(
        field.rules.len(),
        127,
        "source coverage changed; review, do not sample"
    );
    for rule in &field.rules {
        for tuning in [TuningPolicy::Retained24Tet, TuningPolicy::BimbaSpelled24Tet] {
            ...
            for (i, frequency) in expected.iter().enumerate() {
                ...
                assert_eq!(
                    basis.m2["condition"]["musical"]["pitches_hz"][i],
                    *frequency
                );
            }
            assert_eq!(
            ...
            assert_eq!(
                basis.m2["condition"]["source_path"],
                serde_json::to_value(rule).unwrap()
            );
            assert_eq!(
                basis.m2["condition"]["source_revision"],
                field.source_revision
            );
            assert_eq!(
                basis.m2["condition"]["musical"]["tuning"],
                serde_json::to_value(tuning).unwrap()
            );
            assert_eq!(
                basis.m2_input.world_observations.len(),
                request.m2.world_observations.len()
            );
            assert_eq!(
                basis.m2_input.modal_coefficients,
                request.m2.modal_coefficients
            );
            assert_eq!(
                serde_json::to_value(&basis.input).unwrap(),
                serde_json::to_value(&request).unwrap()
            );
            assert_eq!(
                basis.derivation["condition_frequency_bindings"],
                serde_json::to_value(&request.condition_frequency_bindings).unwrap()
            );
            for (original, current) in request
                .m2
                .resonator
                .as_ref()
                .unwrap()
            ...
            for tuning in [TuningPolicy::Retained24Tet, TuningPolicy::BimbaSpelled24Tet] {
                ...
                assert!(
                    request
                        .compose()
                        .unwrap_err()
                        .contains("no admitted correspondence source path")
                );
            }
        }
    }
...
fn dual_bus_validation_has_no_aliases_fallback_or_legacy_reinterpretation() {
    ...
    for case in 0..8 {
        ...
        match case {
            ...
            3 => {
                request.condition_frequency_bindings[0].mode_ref =
                    request.frequency_bindings[0].mode_ref.clone()
            }
            4 => request
                .condition_frequency_bindings
                .push(request.condition_frequency_bindings[0].clone()),
    ...
    assert!(
        basis
            .derivation
            .get("condition_frequency_bindings")
            .is_none()
    );
    assert!(basis.derivation.get("musical_sources").is_none());
    assert_eq!(
        basis.m2_input.resonator.as_ref().unwrap().modes[1].frequency_hz,
        137.0
    );
    assert_eq!(
