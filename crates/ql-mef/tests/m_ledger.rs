...
fn imports_existing_matrix_families_not_a_manual_deep_census() {
    ...
    // infrastructural arena machinery), 179 Rust bodies (including the
    // ql-cli system --json disclosure surface), and one live-graph
    // structural-index record per joined live coordinate (1635).
    assert_eq!(l.implementations.len(), 1904);
