...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let path = std::env::args()
        .nth(1)
        .ok_or("pass the decompressed original property source JSON")?;
    let source: Value = serde_json::from_str(&std::fs::read_to_string(path)?)?;
    let columns = source["columns"]
        .as_array()
        .ok_or("missing source columns")?;
    let rows = source["properties"]
        .as_array()
        .ok_or("missing property definitions")?;
    if rows.len() != 194 {
        return Err("the full original 194-definition vocabulary is required".into());
    }
    ...
    for (index, row) in rows.iter().enumerate() {
        ...
        if values.len() != columns.len() {
            return Err("source property row/column mismatch".into());
        }
        let object: serde_json::Map<String, Value> = columns
            .iter()
            .zip(values)
            .map(|(k, v)| Ok((k.as_str().ok_or("invalid column")?.into(), v.clone())))
            .collect::<std::result::Result<_, &str>>()?;
        let spec: SourcePropertySpec = serde_json::from_value(Value::Object(object))?;
        let dimensions = source["embedding_dimensions"]
            .as_u64()
            .ok_or("missing original embedding dimensions")? as usize;
        let value = sample(&spec, dimensions);
        spec.validate_value(&value, Some(dimensions))?;
        if spec.validate_value(&Value::Null, Some(dimensions)).is_ok() {
            return Err("null type mismatch admitted".into());
        }
        let subject = Subject {
            registry_revision: registry.manifest().registry_revision.clone(),
            owner: spec.owner,
            id: if spec.owner == PropertyOwner::Node {
                registry.root(0).ok_or("M0 absent")?.id
            } else {
        ...
        let definition = Definition {
            ...
            declaration: pin(
                ...
                source["source"]["git_blob"]
                    .as_str()
                    .ok_or("source blob absent")?,
            ),
            applicability_basis: pin("fixture/explicit-native-subject", "1"),
            applies_to: vec![subject.clone()],
            aliases: vec![],
            unit: None,
            domain: vec![],
            embedding_dimensions: if spec.value_type == PropertyType::Embedding {
                Some(dimensions)
            } else {
                None
            },
        ...
        if field.expand(&compact, &FixtureGate)?.reference != receipt.reference {
            return Err("lost source assertion".into());
        }
    }
    if relationship_count != 30 {
        return Err("relationship vocabulary coverage changed".into());
    }
    println!(
        "{}",
        json!({"definition_operations":rows.len(), "node_definitions":164, "relationship_definitions":relationship_count,
