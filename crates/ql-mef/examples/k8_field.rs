...
fn run() -> Result<(), String> {
    ...
    if args.len() != 4 {
        return Err("usage: k8_field WORKER SKY_M2_EVENT INITIAL_FIELD_JSON".into());
    }
    ...
    assert_eq!(
        first["amplitudes_metres"],
        session.read()?["amplitudes_metres"]
    );
    let changed = session.set_axis(
        1,
        LiftInput {
            turns: "-1".into(),
            half_degrees: 37,
    ...
    assert_eq!(
        changed["clock"]["inscription"],
        first["clock"]["inscription"]
    );
    ...
    println!("{quiet}");
    assert_eq!(
        serde_json::to_value(session.original_basis()).unwrap(),
        original
    );
    Ok(())
}
