...
impl<'a> PropertyField<'a> {
    ...
    fn admission<G: NativePropertyAdmission>(
        &self,
        operation: &str,
        payload: &Value,
        gate: &G,
    ...
    fn replay<G: NativePropertyAdmission>(
        &self,
        operation: &str,
        payload: &Value,
        gate: &G,
    ) -> Result<Option<PropertyReceipt>> {
        reference(operation)?;
        if let Some((original, receipt)) = self.operations.get(operation) {
            require(
                original == payload,
                "operation identity reused for different property content",
            )?;
            self.admission(
                "read-operation",
                &json!({"operation": operation, "reference": receipt.reference}),
                gate,
            )?;
        ...
        require(
            self.operations.len() < 16_384,
            "property operation budget reached; native retention required",
        )?;
        Ok(None)
    }
    fn remember(
        ...
        self.operations
            .insert(operation.into(), (payload, receipt.clone()));
        receipt
    }
    fn definition(&self, reference: &VersionRef) -> Result<&Definition> {
        ...
        self.definitions
            ...
            .and_then(|v| v.iter().find(|d| d.reference == *reference))
    ...
    fn assertion(&self, reference: &VersionRef) -> Result<&Assertion> {
        ...
        self.assertions
            .get(&reference.identity)
            .and_then(|v| v.iter().find(|a| a.reference == *reference))
            .ok_or_else(|| "unknown assertion or assertion revision".into())
    }
    pub fn define<G: NativePropertyAdmission>(
        &mut self,
        operation: &str,
        definition: Definition,
        expected: Option<u64>,
        gate: &G,
    ) -> Result<PropertyReceipt> {
        let payload = json!({"kind":"define", "definition":definition, "expected":expected});
        if let Some(result) = self.replay(operation, &payload, gate)? {
            return Ok(result);
        }
        definition.validate(self.registry)?;
        let previous = self
            .definitions
            .get(&definition.reference.identity)
            .and_then(|v| v.last());
        require(
            previous.map(|d| d.reference.revision) == expected,
            "stale property definition revision",
        )?;
        require(
        ...
        if let Some(previous) = previous {
            require(
                previous.spec.key == definition.spec.key
                    && previous.spec.owner == definition.spec.owner,
                "a stable property identity cannot be rebound to another key or owner kind",
            )?;
        }
        for pin in [&definition.declaration, &definition.applicability_basis] {
        ...
        self.definitions
            .entry(definition.reference.identity.clone())
            .or_default()
            .push(definition);
    ...
    fn validate_draft<G: NativePropertyAdmission>(&self, draft: &Draft, gate: &G) -> Result<()> {
        ...
        require(
            self.definitions[&draft.definition.identity]
                .last()
                .map(|d| &d.reference)
        ...
        require(
            definition.applies_to.contains(&draft.subject),
            "property is not applicable to this exact subject",
        )?;
        definition.validate_value(&draft.value)?;
        require(
            !draft.sources.is_empty() && draft.sources.len() <= MAX_REFERENCES,
            "property assertion needs bounded source support",
        )?;
        require(
            draft.standing != Standing::Reviewed,
            "reviewed standing requires an explicit Recognition operation",
        )?;
        ...
        for pin in draft.sources.iter().chain([
            &draft.producer,
            &draft.result,
            &definition.declaration,
            &definition.applicability_basis,
        ]) {
            pin.validate()?;
            gate.check_source(pin)?;
        }
        for pin in &draft.sources {
            require(
                pins.insert((&pin.reference, &pin.revision)),
                "duplicate property source support",
            )?;
        }
        match (&draft.role, &draft.derivation) {
            (ValueRole::GraphDerived, Some(graph)) => {
                require(
                    draft.standing == Standing::Derived || draft.standing == Standing::Proposed,
                ...
                for id in graph.included_nodes.iter().chain(&graph.excluded_nodes) {
                    require(
                        self.registry.node(*id).is_some() && nodes.insert(*id),
                        "unknown, duplicate or included-and-excluded graph subject",
                    )?;
                }
                require(
                    draft.subject.owner != PropertyOwner::Node
                        || graph.included_nodes.contains(&draft.subject.id),
                    "derived subject is not inside its graph projection",
                )?;
            }
            (ValueRole::GraphDerived, None) => {
                return Err(
                    "graph-derived property requires actual graph/algorithm provenance".into(),
                );
            }
            (_, Some(_)) => {
                return Err(
                    "graph derivation cannot be disguised as an intrinsic/quintessential value"
                        .into(),
                );
            }
            _ => {}
    ...
    pub fn publish<G: NativePropertyAdmission>(
        &mut self,
        operation: &str,
        draft: Draft,
        expected: Option<u64>,
        ...
        require(
            previous.map(|a| a.reference.revision) == expected,
            "stale assertion revision",
        )?;
        if let Some(previous) = previous {
            require(
                previous.content.subject == draft.subject
    ...
    pub fn recognise<G: NativePropertyAdmission>(
        &mut self,
        operation: &str,
        current: &VersionRef,
        recognition: SourcePin,
        gate: &G,
    ) -> Result<PropertyReceipt> {
        ...
        require(
            self.assertions[&current.identity]
                .last()
                .map(|a| &a.reference)
                == Some(current),
            "Recognition uses a superseded assertion",
        )?;
        require(
            next.content.standing != Standing::Reviewed,
            "assertion is already reviewed",
        )?;
    ...
    pub fn rollback<G: NativePropertyAdmission>(
        &mut self,
        operation: &str,
        current: &VersionRef,
        restore: &VersionRef,
        ...
        require(
            current.identity == restore.identity && restore.revision < current.revision,
    ...
    pub fn read<G: NativePropertyAdmission>(
        &self,
    ...
    fn compact_value(&self, assertion: &Assertion, maximum_bytes: usize) -> Result<Compact> {
        require(
            serde_json::to_vec(&assertion.content.value)
        ...
        Ok(Compact {
            ...
            assertion: assertion.reference.clone(),
            definition: assertion.content.definition.clone(),
            subject: assertion.content.subject.clone(),
            value: assertion.content.value.clone(),
            role: assertion.content.role,
            standing: assertion.content.standing,
            support: assertion.content.result.clone(),
        })
    }
    pub fn compact<G: NativePropertyAdmission>(
        &self,
        reference: &VersionRef,
        maximum_bytes: usize,
        gate: &G,
    ...
    pub fn expand<G: NativePropertyAdmission>(
        &self,
        compact: &Compact,
        gate: &G,
    ) -> Result<&Assertion> {
        ...
        require(
            self.compact_value(rich, MAX_VALUE_BYTES)? == *compact,
            "compact property differs from its exact rich assertion",
        )?;
        Ok(rich)
    }
    pub fn assertions_for<G: NativePropertyAdmission>(
        &self,
        subject: &Subject,
        gate: &G,
        ...
        self.assertions
            .values()
            .filter_map(|v| v.last())
            .filter(|a| a.content.subject == *subject)
            .map(|a| self.read(&a.reference, gate))
            .collect()
    }
    /// Ambiguous aliases return every independently source-owned candidate.
    pub fn definitions_named<G: NativePropertyAdmission>(
        &self,
        name: &str,
        gate: &G,
        ...
        let values: Vec<_> = self
            ...
            .filter_map(|v| v.last())
            .filter(|d| {
                d.reference.identity == name
                    || d.spec.key == name
                    || d.aliases.iter().any(|a| a == name)
            })
            .collect();
        for definition in &values {
            self.admission("read-definition", &json!(definition), gate)?;
        }
        Ok(values)
    }
}
