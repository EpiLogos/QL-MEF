    ...
    pub fn snapshot(&self) -> Value {
        ...
        json!({"schema":STATE_SCHEMA,"registry_revision":self.request.registry_revision,"domain_revision":source.revision(),"source_revision":source.source_revision(),
            ...
            "form":{"codon":ref_value(M3NodeKind::Codon,c.address() as usize),"hexagram":ref_value(M3NodeKind::Hexagram,c.address() as usize),
                ...
                "physical_form":{"schema":"ql.m3-physical-form-target/v1","target_kind":"fold-pose",
                    "constituent_ref":engine.node(M3NodeKind::Codon,c.address() as usize).unwrap().source_ref,
                    "pose_ordinal":self.fold.rotational_pose().ordinal(),"state_count":self.fold.state_count(),"address":c.address(),
                    "standing":"explicit-m3-physical-form-target/v1; not orientation_seed; not presentation glyph; not source angles as pose"}},
