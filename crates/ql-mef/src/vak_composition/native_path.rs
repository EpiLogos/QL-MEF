...
impl VakComposition {
    pub fn record_native_observation(
        &mut self,
        registry: &VakRegistry,
        input: NativePathInput,
    ) -> Result<NativePathCorrelation> {
        reference(&input.reference)?;
        reference(&input.native_step_ref)?;
        input.basis.validate()?;
        let d = self.determination(&input.determination)?;
        require(
            d.native_paths.len() < MAX_DEPTH,
            "native observation correlation bound exceeded",
        )?;
        require(
            !d.native_paths
                .iter()
                .any(|p| p.reference == input.reference),
            "native correlation already exists",
        )?;
        let language = d.language.as_ref().ok_or_else(|| {
            err("native correlation requires the producing full-profile expression")
        })?;
        let contribution = d
            .contribution
            .as_ref()
            .ok_or_else(|| err("native generation requires attributable Agent input"))?;
        language.validate(registry)?;
        require(
            input.observation.actor_ref.as_deref() == Some(&contribution.actor_ref),
            "native observation actor differs from producing Agent",
        )?;
        let general = &input.observation.general_expression;
        require(
            general.syntax_version == language.general.syntax_version
        ...
        require(
            input.observation.world_ref == language.reading.world_ref
        ...
        for step in &input.observation.steps {
            require(
                ids.insert(step.step_id.clone()),
                "native path has duplicate step identities",
            )?;
            references(&step.evidence_refs)?;
            for r in step
                .invocation_ref
                .iter()
                .chain(step.activity_ref.iter())
                .chain(step.return_ref.iter())
                .chain(step.result_refs.iter())
            {
                reference(r)?;
            }
        }
        let path = reconstruct_observed_vak_path_for_revision(
            registry,
            input.observation,
            &language.accepted_syntax_revision,
        )
        .map_err(err)?;
        let step = path
            .steps
            .iter()
            .find(|s| s.step_id == input.native_step_ref)
            .ok_or_else(|| err("native step is not in the supplied path"))?;
        require(
            step.expression.operator == language.reading.operator
        ...
        require(
            step.result_refs.contains(&contribution.result_ref)
        ...
        let value = NativePathCorrelation {
            ...
            native_node_ref: language.native_node_ref.clone(),
            native_step_ref: input.native_step_ref,
            path,
            basis: input.basis,
            standing: VakStanding::Derived,
        };
        self.determinations
            .get_mut(&input.determination)
            .expect("validated determination")
            .native_paths
            .push(value.clone());
        Ok(value)
    }
