...
mod tests {
    ...
    fn all_thread_forms_have_distinct_executable_obligations() {
        let single = plan(&["one"]);
        ThreadForm::Single.validate_plan("concern", &single).unwrap();
        let parallel = plan(&["one", "two"]);
        ThreadForm::Parallel.validate_plan("concern", &parallel).unwrap();
    ...
    fn lost_chain_input_and_fused_subject_changes_are_rejected() {
        ...
        assert!(ThreadForm::Fusion.validate_plan("concern", &fusion).is_err());
        fusion.aggregation_ref = Some("aggregate".into());
        fusion.legs[1].subject_ref = "another-concern".into();
        assert!(ThreadForm::Fusion.validate_plan("concern", &fusion).is_err());
        assert!(ThreadForm::Sustained.validate_plan("concern", &plan(&["one"])).is_err());
    }
    #[test]
    fn malformed_or_aliased_native_topology_fails_closed() {
        ...
        assert!(ThreadForm::Nested.validate_plan("concern", &parent_cycle).is_err());
        let mut alias = plan(&["one", "two"]);
        alias.legs[1].result_ref = alias.legs[0].result_ref.clone();
        assert!(ThreadForm::Parallel.validate_plan("concern", &alias).is_err());
        assert!(ThreadForm::Single.validate_plan("concern", &plan(&[])).is_err());
        let mut unknown = plan(&["one"]);
        unknown.legs[0].after.push("absent".into());
        assert!(ThreadForm::Single.validate_plan("concern", &unknown).is_err());
    }
    #[test]
