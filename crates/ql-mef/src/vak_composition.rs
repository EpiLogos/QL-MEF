...
use ql_core::{
    ...
    QlPosition, QlShape, QlShapeAddress, QlShapeCompression,
    RELATION_FIELD_COMPOSITION_OPERATOR_REF, RelationFieldComposition, RelationFieldDerivation,
    ShapeBinding, StructuralConstellation, StructuralParticipation, VakFamily,
};
    ...
    pub fn bind_whole(&mut self, registry: &VakRegistry, input: WholeInput) -> Result<()> {
        ...
        require(
            input.binding.whole_ref == form.anchor_ref
                && binding_shape_matches_form(&input.binding, &form),
        ...
        *budget -= 1;
        ...
        let (address, children, carrier_derivations) = match &whole.body {
            WholeBody::Local(form) => {
                let compressed = matching_compression(&whole.binding, form).is_some();
                let a = if form.members.is_empty()
                    || (compressed
                        && whole.member_focus.is_none()
                        && whole.selected_member.is_none())
                {
...
/// The kernel-validated compressed reading, when the binding presents a real
/// disclosed 2/3-fold constellation as the one-fold `0/1` whole. The binding
/// must carry the exact compression operator and derivation generated by the
/// same QL shape grammar; arbitrary shape-ref mismatches remain invalid.
fn matching_compression(
    binding: &ShapeBinding,
    form: &StructuralConstellation,
) -> Option<QlShapeCompression> {
    let disclosed = QlShape::Constellation(form.grain());
    let compression = QlShapeCompression::to_onefold(disclosed)?;
    let derivation_ref = compression.derivation_ref();
    (binding.shape_ref == compression.presented.shape_ref()
        && binding.operator_ref.as_deref() == Some(compression.operator_ref)
        && binding.derivation_ref.as_deref() == Some(derivation_ref.as_str()))
    .then_some(compression)
}

fn binding_shape_matches_form(binding: &ShapeBinding, form: &StructuralConstellation) -> bool {
    let disclosed = QlShape::Constellation(form.grain());
    binding.shape_ref == disclosed.shape_ref() || matching_compression(binding, form).is_some()
}
