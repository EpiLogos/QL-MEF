...
impl<'a> M3Engine<'a> {
    pub fn new(registry: &'a MRegistry) -> Result<Self, String> {
        ...
        let mut halves: BTreeMap<MTreeId, BTreeMap<&str, BTreeSet<MTreeId>>> = BTreeMap::new();
        for relation in &registry.manifest().relations {
            if matches!(
                ...
                if registry
                    .node(from)
                    .is_some_and(|n| n.source_ref.starts_with("#3-1-"))
                {
                    halves
                        .entry(from)
                        .or_default()
                        .entry(&relation.source_kind)
                        .or_default()
                        .insert(to);
                }
            }
        }
        let mut trigram_bits = BTreeMap::new();
        for t in Trigram::LUT {
            trigram_bits.insert(lookup(format!("#3-1-{}", t.id))?, t.binary);
        }
        for (id, parts) in halves {
            let bits = |kind| -> Result<u8, String> {
                let targets = parts.get(kind).ok_or("missing trigram relation")?;
                if targets.len() != 1 {
                    return Err("ambiguous trigram relation".into());
                }
                trigram_bits
        ...
        if hexagrams.len() != 64 {
            return Err("missing relation-derived hexagram".into());
        }
        ...
        for kind in M3NodeKind::ALL {
            ...
            for index in 0..kind.count() {
                let reference = match kind {
                    ...
                    M3NodeKind::Hexagram => registry
                        .node(hexagrams[&(index as u8)])
                        .ok_or("missing hexagram node")?
                        .source_ref
                        .clone(),
        ...
        for relation in &registry.manifest().relations {
            if let (Some(from), Some(to)) = (relation.from_id, relation.to_id) {
                edges
                    .entry((&relation.source_kind, from))
                    .or_default()
                    .insert(to);
            }
        }
        let degrees = &groups[M3NodeKind::Degree as usize];
        let backbone: BTreeSet<_> = groups[M3NodeKind::Backbone as usize]
            .iter()
            .copied()
            .collect();
        ...
        for (degree, id) in degrees.iter().enumerate() {
            ...
            for (kind, offset) in [("FLOWS_CLOCKWISE", 1), ("POLAR_OPPOSITE", 180)] {
                if edges.get(&(kind, *id))
                    != Some(&BTreeSet::from([degrees[(degree + offset) % 360]]))
                {
                    return Err(format!(
                        "clock source/arithmetic discrepancy: {kind}, {degree}"
                    ));
                }
            }
            anchors.push(anchor);
        ...
        if backbone
            .iter()
            .any(|id| anchors.iter().filter(|a| *a == id).count() != 15)
        {
    ...
    pub fn coordinates(&self) -> impl Iterator<Item = &'a MTreeNode> {
        self.registry
            ...
            .nodes
            .iter()
            .filter(|n| n.root_position == Some(3))
    }
    ...
    pub fn clock(&self, clock: M3Clock) -> M3ClockFrame<'a> {
        ...
        M3ClockFrame {
            clock,
            degree: self.node(M3NodeKind::Degree, d).unwrap(),
...
pub fn native_m3_engine() -> &'static M3Engine<'static> {
    ...
    ENGINE.get_or_init(|| {
        M3Engine::new(native_m_registry()).expect("accepted M3 source relation field")
    })
}
