...
def materialize(previous):
    ...
    result=m.refresh(ROOT,previous)
    ...
    for row in sources:
        if row['assessment'] != 'unassessed' and not row['assessment'].startswith(PREFIX):
            raise ValueError('M2 source row has a later external assessment; reconcile explicitly: '+row['id'])
        ...
        row['dependencies']=list(dict.fromkeys([d for d in row['dependencies'] if not d.startswith(PREFIX)]+[r['id'] for r in relevant]+[row_id('engine')]))
        row['bindings']=[b for b in row['bindings'] if not b.startswith(PREFIX)]+[b for r in relevant for b in r['bindings']]
        ...
        row['dispositions'].update({peer:'bound' if indexed[peer] else 'planned' for peer in ['c','rust']})
        row['dispositions'].update({'cpp':'planned','neo4j':'planned'})
    differences=[
      ('k4','source','bimba','rust','K4 whole-programme acceptance was not observed: #128 still carries K3-to-K4 handoff. K6 supplies a bounded M2 census, not retrospective K4 acceptance.'),
...
def proof_refresh():
    ...
    parity=load('target/m2-receipt/rust-parity.json');vim=load('target/m2-receipt/vimarsha-parity.json')
    if parity['result']!='pass' or vim['result']!='passed':raise ValueError('missing executed peer proof')
    write(PROOF,{'schema':'ql.m2-finite-proof/v1','result':'passed','standing':'exact input hashes executed; final CI acceptance.json binds execution to the accepted Git revision',
        'inputs':receipt['inputs'],'finite_observation':parity,'vimarsha_observation':vim,'checks':receipt['checks'],
...
def main():
    ...
    for key,path in [('finite_observation','rust-parity.json'),('vimarsha_observation','vimarsha-parity.json')]:
